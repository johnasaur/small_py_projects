...
def start_timer():
    ...
    s_short_break = SHORT_BREAK_MIN * 60
    s_long_break = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        count_down(s_long_break)
        title_label.config(text="BREAK", fg=RED)
    elif reps % 2 == 0:
        count_down(s_short_break)
...
def count_down(count):
    ...
    if count > 0:
        ...
        start_timer()
        mark = ""
        for _ in range(math.floor(reps/2)): #work sessions
            mark += "v"
